...
export async function registerSource(details: any, type: 'telegram' | 'web') {
  ...
  if (details.url) {
    const { data: urlCheck } = await supabase.from('ideas').select('id').eq('original_url', details.url).limit(1)
  ...
  if (extractedIdeas.length > 0) {
    const needsTranslate = extractedIdeas.some(
      (idea: any) => /[A-Za-z]/.test(idea.title || '') && !/[А-Яа-яЁё]/.test(idea.title || '')
    )
    if (needsTranslate) {
      try {
  ...
  for (const idea of extractedIdeas) {
    ...
    const { data: existing } = await supabase.from('ideas').select('id').eq('title', idea.title.trim()).limit(1)
