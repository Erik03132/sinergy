...
import { VERTICAL_COMPATIBILITY, SYNERGISTIC_TECH_PAIRS } from './constants'
...
export function isVerticalCompatible(a: Idea, b: Idea): boolean {
  ...
  return (
    VERTICAL_COMPATIBILITY[a.vertical || '']?.includes(b.vertical || '') ||
    VERTICAL_COMPATIBILITY[b.vertical || '']?.includes(a.vertical || '') ||
    false
  )
}
...
export function calculateBlueOceanPotential(a: Idea, b: Idea): number {
  ...
  const audienceSpecificity =
    (a.target_audience && !['users', 'businesses'].includes(a.target_audience.toLowerCase()) ? 0.5 : 0) +
  ...
  const hasSimplification = simplificationKeywords.some(
    (kw) => (a.description || '').toLowerCase().includes(kw) || (b.description || '').toLowerCase().includes(kw)
  )
  ...
  const isRare = rareCombos.some(
    (combo) =>
      (a.vertical === combo.v1 && b.vertical === combo.v2) || (a.vertical === combo.v2 && b.vertical === combo.v1)
  )
  score += isRare ? 3 : 1
